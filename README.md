...
Every particle keeps its own speed but continuously bends its heading toward the
nearest detected landmark (any of the 21 points, on up to two hands), so the
whole field rushes toward your hand. When no hand is visible the particles fall
straight back to their default meander. Needs a secure context (works on
`localhost`).
